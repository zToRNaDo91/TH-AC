...
BlacklistedWeapons = { -- Wenn jemand die Waffen hat wird er gebannt!
	"WEAPON_BALL",
	"WEAPON_RAILGUN",
	"WEAPON_GARBAGEBAG",
}
...
Citizen.CreateThread(function()
	...
	while true do
		...
		SetEntityVisible(PlayerPedId(), true) -- macht den Spieler Sichtbar!
		Wait(3000) -- warte 3 sekunden und dann wird alles nochmal geprüft / ausgeführt

		local mehr = speed - 9.0 -- Die durchschnittliche Laufgeschwindigkeit beträgt 7.06. Wenn also jemand etwas schneller läuft, wird dies nicht ausgelöst!

		local rounds = tonumber(string.format("%.2f", speed))
		local roundm = tonumber(string.format("%.2f", mehr))


		if not IsEntityVisible(PlayerPedId()) then
			SetEntityHealth(PlayerPedId(), -100) -- Wenn der Spieler unsichtbar ist, wird er getötet!
		end
		...
		if GetDistanceBetweenCoords(posx,posy,posz, newx,newy,newz) > 200 and still == IsPedStill(ped) and vel == GetEntitySpeed(ped) and ped == newPed then
			TriggerServerEvent("thac:NoclipFlag", GetDistanceBetweenCoords(posx,posy,posz, newx,newy,newz))
		end
		...
		if speed > 9.0 and not veh and (para == -1 or para == 0) and not flyveh and not fall and not parafall and not rag then
			...
			--TriggerServerEvent("thac:SpeedFlag", rounds, roundm) -- send alert along with the rounded speed and how much faster they are
		end
	end
end)
	...
	while true do
		...
		if not IsPlayerDead(PlayerId()) then
			if PlayerPedId() == curPed and GetEntityHealth(curPed) == curHealth and GetEntityHealth(curPed) ~= 0 then
				TriggerServerEvent("thac:HealthFlag", false, curHealth-2, GetEntityHealth( curPed ),curWait )
			elseif GetEntityHealth(curPed) == curHealth-2 then
				SetEntityHealth(curPed, GetEntityHealth(curPed)+2)
			end
		end
		if GetEntityHealth(curPed) > 400 then
			TriggerServerEvent("thac:HealthFlag", false, GetEntityHealth( curPed )-200, GetEntityHealth( curPed ),curWait )
		end
		...
		if GetPlayerInvincible( PlayerId() ) then -- if the player is invincible, flag him as a cheater and then disable their invincibility
			TriggerServerEvent("thac:HealthFlag", true, curHealth-2, GetEntityHealth( curPed ),curWait )
	...
	while true do
		...
		for _,theWeapon in ipairs(BlacklistedWeapons) do
			...
			if HasPedGotWeapon(PlayerPedId(),GetHashKey(theWeapon),false) == 1 then
					TriggerServerEvent("thac:WeaponFlag", theWeapon)
					break
			end
		end
	end
...
RegisterNetEvent("thac:RemoveInventoryWeapons")
AddEventHandler('thac:RemoveInventoryWeapons', function()
	...
	while true do
		...
		if IsPedJumping(PlayerPedId()) then
			...
			if jumplength > 250 then
				TriggerServerEvent("thac:JumpFlag", jumplength )
			end
		end
	end
end)
